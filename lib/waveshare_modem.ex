defmodule WaveshareModem do
  ...
  def init(_init_state) do
    ...
    Logger.info("[AT Modem] modem status: #{modem_status}")
  ...
  @impl GenServer
  def handle_call({:play_tone, tone}, _from, %{uart_pid: uart_pid} = state) do
    response = do_play_tone(uart_pid, tone)

    Logger.info("[AT Modem] response to play tone: #{inspect(response)}")
    state = Map.put(state, :modem_status, :playing_tone)

    {:reply, :ok, state}
  end

  def handle_call({:play_ext_tone, tone}, _from, %{uart_pid: uart_pid} = state) do
    response = do_play_ext_tone(uart_pid, tone)

    Logger.info("[AT Modem] response to play ext tone: #{inspect(response)}")
    state = Map.put(state, :modem_status, :playing_tone)

    {:reply, :ok, state}
  end

  def handle_call({:cancel_ext_tone}, _from, %{uart_pid: uart_pid} = state) do
  ...
  @impl GenServer
  def handle_call(:hang_up, {_pid, _reference}, %{uart_pid: uart_pid} = state) do
    Logger.info("[AT Modem] handle call :hangup")
    do_hang_up(uart_pid)
  ...
  def handle_call(
    ...
    state =
      if String.length(phone_number) >= 7 do
        Logger.info("[AT Modem] handle call :make_phone_call")
        {:ok, response} = make_voice_call(uart_pid, phone_number)
        Logger.info("[AT Modem] voice call response: #{inspect(response)}")
        Map.put(state, :modem_status, :on_voice_call)
      else
        Logger.info("[AT Modem] Mistakes were made.  Don't call phone number: #{phone_number}")
  ...
  defp do_play_tone(uart_pid, tone) do
    Logger.info("[AT Modem] playing tone: #{tone}")

    {:ok, response} =
      send_command_get_response(uart_pid, "AT+CPTONE=#{tone},#{@tone_duration},#{@tone_gain}\r\n")
  ...
  defp do_hang_up(uart_pid) do
    Logger.info("[AT Modem] hanging up")
  ...
  defp set_loudspeaker_volume(uart_pid, volume) do
    Logger.info("[AT Modem] set loudspeaker volume to: #{volume}")
  ...
  defp send_command_get_response(uart_pid, command) do
    ...
    if response1 != "" do
      Logger.info("[AT Modem] command: #{command} 1st response: #{inspect(response1)}")
    end
